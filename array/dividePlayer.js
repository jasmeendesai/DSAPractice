...
const dividePlayer = (skill) => {
    ...
    if(totalSum % countOfGroup !==0) return -1
    ...
    for(let i=0; i<n; i++){
        let compliment = eachGroupSum - skill[i]

        if(!map.has(compliment) || map.get(compliment) <=0) return -1

        sumOfGroupProduct += compliment * skill[i]
        map.set(compliment, (map.get(compliment)-1))
    }
    return sumOfGroupProduct/2
}

console.log(dividePlayer(skill))